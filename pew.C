...
void translate_word_command(){ char word[128]; char lang[16];
    ...
    if(!lang[0]){
        if(isatty(fileno(stdin))){ prompt_translate(word); return; }
...
const char *lookup_glossary(const char *term){
    for(int i=0;i<sizeof(glossary)/sizeof(glossary[0]);i++){
        if(strcasecmp(glossary[i].term, term)==0) return glossary[i].explanation;
    }
    return NULL;
}
...
void animate_bird_death(){
    if(!isatty(fileno(stdout))){ printf("pew is terminated.\n"); return; }
    const char *frames[] = {
        "\033[1;32m   ,~.   \n  (  0)  \n   `-'   \033[0m\n",
        "\033[1;31m   ,~.   \n  ( x)   \n   `-'   \033[0m\n",
        "\033[1;31m   .-.   \n  ( x)   \n   /|\\  \033[0m\n",
...
Node* parse_primary(){ skipws(); if(match("+")) return parse_primary(); if(match("-")){ return mknode(NODE_SUB, mknum(0), parse_primary()); }
	...
	// dy/dx(expression)
	if(strncasecmp(&src[pos],"dy/d",4)==0){ // accept dy/dx
	...
	// int(expr, x)
	if(strncasecmp(&src[pos],"int",3)==0){ pos+=3; skipws(); if(src[pos]=='(') pos++; Node *ex = parse_expr(); skipws(); if(src[pos]==',') pos++; skipws(); char var = src[pos]; if(isalpha((unsigned char)var)) pos++; skipws(); if(src[pos]==')') pos++;
		Node *res = integrate_symbolic(ex,var);
		if(res) return res;
		// fallback: return numeric integration indicator as 0
		return mknum(0);
	}
	if(strncasecmp(&src[pos],"def.int",7)==0){ pos+=7; skipws(); if(src[pos]=='(') pos++; Node *ex = parse_expr(); skipws(); if(src[pos]==',') pos++; skipws(); char var = src[pos]; if(isalpha((unsigned char)var)) pos++; skipws(); if(src[pos]==',') pos++; skipws(); // parse a
		// read numbers
...
void handle_line(const char *line){ src=line; pos=0; skipws();
    ...
    if(match("mode") || match("set")){
        ...
        if(strncasecmp(&src[pos],"binary",6)==0){ num_mode=2; printf("mode set to binary\n"); }
        else if(strncasecmp(&src[pos],"ternary",7)==0){ num_mode=3; printf("mode set to ternary\n"); }
        else if(strncasecmp(&src[pos],"decimal",7)==0){ num_mode=10; printf("mode set to decimal\n"); }
...
int main(int argc, char **argv){
	welcome_animation();
	printf("Welcome to pew — a small expressive shell language with math, game commands, and creative definitions.\n");
